        ...
        function fieldDirective($rootScope, $sce, $document, $translate, SpreadsheetService, DataManipulationService,
            ...
            var linker = function ($scope, $element, attrs) {
                ...
                if ($scope.directory == 'render') {
                    console.log('Added by cedar');
                    if ($scope.model) {
                        if ($rootScope.isArray($scope.model)) {
                            if ($scope.model.length == 0) {
                                var min = $scope.field.minItems || 0;

                                if (field._valueConstraints && field._valueConstraints.literals.defaultOption) {
                                    for (var i = 0; i < min; i++) {
                                        $scope.model[i]["_value"] = angular.copy(field._valueConstraints.literals.defaultOption);
                                    }
                                } else {
                                    for (var i = 0; i < min; i++) {
                            ...
                            if (!("_value" in $scope.model)) {
                                console.log('no value in model1');
                                console.log(field);

                                if (field._valueConstraints && field._valueConstraints.literals.defaultOption) {
                                    console.log('copy default');
                                    $scope.model["_value"] = angular.copy(field._valueConstraints.literals.defaultOption);
                                } else {
                                    if (['checkbox'].indexOf(field.inputType) >= 0 ||
                                        ['date'].indexOf(field.inputType) >= 0 && field.dateType == "date-range") {
                                        $scope.model['_value'] = {};
                ...
                $scope.addMoreInput = function () {
                    if ((!$scope.field.maxItems || $scope.model.length < $scope.field.maxItems)) {
                        ...
                        if (field._valueConstraints.literals.defaultOption) {
                            seed["_value"] = angular.copy(field._valueConstraints.literals.defaultOption);
                        } else {
                            if (['checkbox'].indexOf(field.inputType) >= 0 ||
                                ['date'].indexOf(field.inputType) >= 0 && field.dateType == "date-range") {
                                seed['_value'] = {};
                ...
                $scope.checkFieldConditions = function (field) {
                    ...
                    if (extraConditionInputs.indexOf(field._ui.inputType) !== -1) {
                        ...
                        angular.forEach(field._valueConstraints.literals.options, function (value, index) {
                    ...
                    if ((field._ui.inputType == 'radio' || field._ui.inputType == 'list') && field._valueConstraints.literals.options && (field._valueConstraints.literals.options.length <= 1)) {
                ...
                $scope.$watch("model", function () {
                    ...
                    $scope.refreshOption = function (index) {
                        ...
                        // make a deep copy
                        var options = JSON.parse(JSON.stringify($rootScope.schemaOf($scope.field)._valueConstraints.literals.options));
                        for (var i = index; i < options.length; i++) {
                            // empty out the item and following items
                            $rootScope.schemaOf($scope.field)._valueConstraints.literals.options[i] = {};
                        }
                        $timeout(function () {
                            $rootScope.schemaOf($scope.field)._valueConstraints.literals.options = options;
                        });
                    };
                    ...
                    if ($scope.directory == 'render') {
                        console.log('If a default value is set from the field item configuration, set $scope.model to its value');
                        ...
                        if ($scope.model) {
                            if ($rootScope.isArray($scope.model)) {
                                ...
                                if (!("_value" in $scope.model)) {
                                    console.log('no value in model2');
                                    console.log('field._valueConstraints');

                                    if (field._valueConstraints && field._valueConstraints.literals.defaultOption) {
                                        console.log('copy over defaults');
                                        $scope.model["_value"] = angular.copy(field._valueConstraints.literals.defaultOption);
                                    } else {
                                        if (['checkbox'].indexOf(field.inputType) >= 0 ||
                                            ['date'].indexOf(field.inputType) >= 0 && field.dateType == "date-range") {
                ...
                $scope.initRadioDefaultOption = function () {
                    ...
                    if (!$rootScope.schemaOf($scope.field)._valueConstraints.literals.hasOwnProperty('defaultOption')) {
                        console.log($rootScope.schemaOf($scope.field)._valueConstraints.literals.options[0]['text']);
                        $rootScope.schemaOf($scope.field)._valueConstraints.literals.defaultOption = $rootScope.schemaOf($scope.field)._valueConstraints.literals.options[0]['text'];
                    }

                }

                $scope.initDefaults = function (model) {

                    if (model._value == "") {
                        if ($rootScope.schemaOf($scope.field)._valueConstraints.literals.hasOwnProperty('defaultOption')) {
                            console.log('set model default values');
                            if ($rootScope.schemaOf($scope.field)._ui.inputType == 'radio') {
                                model._value = $rootScope.schemaOf($scope.field)._valueConstraints.literals.defaultOption;

                            } else {
                                if ($rootScope.schemaOf($scope.field)._ui.inputType == 'checkbox' || $rootScope.schemaOf($scope.field)._ui.inputType == 'list' ) {
                                    model._value = $rootScope.schemaOf($scope.field)._valueConstraints.literals.defaultOption;
